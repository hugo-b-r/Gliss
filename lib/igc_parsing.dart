...
class GNSSFix {
  ...
  GNSSFix(this.rawtime, this.lat, this.lon, this.validity, this.pressAlt,
      this.gnssAlt, this.extras) {
    flight = null;
  ...
  GNSSFix.buildFromBRecord(String bRecordLine, int index) {
    var regexp = RegExp(
        r"^B(\d\d)(\d\d)(\d\d)(\d\d)(\d\d)(\d\d\d)([NS])(\d\d\d)(\d\d)(\d\d\d)([EW])([AV])([-\d]\d\d\d\d)([-\d]\d\d\d\d)([0-9a-zA-Z\-]*).*$");
    var match = regexp.firstMatch(bRecordLine)!;
    ...
    this.pressAlt = double.parse(pressAlt);
    this.gnssAlt = double.parse(gnssAlt);
  }
...
class Glide {
  ...
  Glide(this.enterFix, this.exitFix, this.trackLength);
...
class FlightParsingConfig extends Object {
  ...
  // Soft limit, some fixes are allowed to exceed.
  var minSecondsBetweenFixes = 1.0;
...
class Flight {
  ...
  void _parseHRecord(String record) {
    if (record.substring(0, 5) == "HFDTE") {
      var match = RegExp('(?:HFDTE|HFDTEDATE:[ ]*)(\d\d)(\d\d)(\d\d)',
          caseSensitive: false);
      if (match.hasMatch(record)) {
        ...
        if (1 <= month && month <= 12 && 1 <= day && day <= 31) {
          ...
          dateTimestamp = (date.millisecondsSinceEpoch ~/ 1000);
        }
      }
      ...
      var match2 = RegExp('HFR[FH]W[ ]*HARDWARE[ ]*VERSION[ ]*:[ ]*(.*)',
          caseSensitive: false);
      if (match.hasMatch(record)) {
        var match3 = match2.firstMatch(record);
        if (match3 != null) {
  ...
  void _checkFixRawtime() {
    ...
    for (var i = 1; i < fixes.length; i++) {
      ...
      if (timeChange < _config.minSecondsBetweenFixes) {
  ...
  void _computeGroundSpeeds() {
    fixes[0].gsp = 0.0;
    for (int i = 1; i < fixes.length; i++) {
      var dist = fixes[i].distanceTo(fixes[i - 1]);
  ...
  ///       learning.
  List<int> _flyingEmissions() {
    List<int> emissions = [];
    for (var fix in fixes) {
      if (fix.gsp > _config.minGspFlight) {
        emissions.add(1);
      } else {
        emissions.add(0);
      }
...
// }
  ...
  List<LatLng> toListLatLng() {
    List<LatLng> listLatLng = [];
    for (var fix in fixes) {
      listLatLng.add(fix.toLatLng());
    }
    return listLatLng;
  }
  ...
  LatLngBounds toLatLngBounds() {
    return LatLngBounds.fromPoints(toListLatLng());
  }
}
